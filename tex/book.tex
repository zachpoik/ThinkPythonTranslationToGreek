...
\index{index}
Ένα λεξικό μοιάζει με μία λίστα, αλλά είναι πιο γενικό. Σε μία λίστα, οι δείκτες πρέπει να είναι ακέραιοι αριθμοί αλλά σε ένα λεξικό μπορούν να είναι σχεδόν οποιουδήποτε τύπου.

Μπορείτε να φανταστείτε ένα λεξικό σαν μία αντιστοίχιση μεταξύ ενός συνόλου δεικτών (οι οποίοι ονομάζονται κλειδιά) και ενός συνόλου τιμών. Κάθε κλειδί αντιστοιχεί σε μία τιμή. Αυτή η σχέση  ονομάζεται  " ζευγάρι κλειδιού-τιμής "  ή μερικές φορές  " αντικείμενο ". 

 Σαν παράδειγμα, θα φτιάξουμε ένα λεξικό το οποίο θα αντιστοιχίζει Αγγλικές με Ισπανικές λέξεις. Επομένως τόσο τα κλειδιά όσο και οι τιμές θα είναι όλα συμβολοσειρές.

Η συνάρτηση  {\tt dict}  δημιουργεί ένα νέο λεξικό χωρίς καθόλου αντικείμενα. Επειδή το  {\tt dict}  είναι το όνομα μίας ενσωματωμένης συνάρτησης θα πρέπει να αποφεύγετε να το χρησιμοποιείτε σαν όνομα μεταβλητής.
\index{dict function}
\index{function!dict}
...
\end{verbatim}
 ...
 Τα άγκιστρα  \verb"{}"  αναπαριστούν ένα κενό λεξικό. Για να προσθέσετε αντικείμενα στο λεξικό μπορείτε να χρησιμοποιήσετε αγκύλες :
...
\end{verbatim}
%
 Αυτή η γραμμή δημιουργεί ένα αντικείμενο αντιστοιχίζοντας το κλειδί  {\tt 'one'}  με την τιμή  \verb"'uno'".  Αν επανεμφανίσουμε το λεξικό τότε θα δούμε ένα ζευγάρι κλειδιού-τιμής με μία άνω κάτω τελεία μεταξύ του κλειδιού και της τιμής :
...
\end{verbatim}
%
 Αυτή η μορφή εξόδου είναι επίσης και μία μορφή εισόδου. Για παράδειγμα, μπορείτε να δημιουργήσετε ένα νέο λεξικό με τρία αντικείμενα με αυτόν τον τρόπο :

\begin{verbatim}
>>> eng2sp = {'one': 'uno', 'two': 'dos', 'three': 'tres'}
\end{verbatim}
%
 Αλλά μάλλον θα εκπλαγείτε αν εμφανίσετε το  {\tt eng2sp}:
...
\end{verbatim}
%
 Η σειρά των ζευγαριών κλειδιού-τιμής είναι διαφορετική. Στην πραγματικότητα, αν πληκτρολογήσετε το ίδιο παράδειγμα στον υπολογιστή σας μάλλον θα έχετε διαφορετικό αποτέλεσμα. Σε γενικές γραμμές, η σειρά των αντικειμένων σε ένα λεξικό απρόβλεπτη. 

Αλλά αυτό δεν αποτελεί πρόβλημα επειδή τα αντικείμενα ενός λεξικού δεν ευρετηριάζονται 
με ακέραιους δείκτες. Αντ' αυτού, μπορείτε να χρησιμοποιήσετε τα κλειδιά για να αναζητήσετε τις αντίστοιχες τιμές :
...
\end{verbatim}
%
 Το κλειδί  {\tt 'two'}  αντιστοιχεί πάντα στην τιμή  \verb"'dos'",  άρα η σειρά των αντικειμένων δεν έχει σημασία.
...
\end{verbatim}
%
 Για να δείτε αν κάτι εμφανίζεται σαν τιμή σε ένα λεξικό μπορείτε να εφαρμόσετε τη μέθοδο  {\tt values},  η οποία επιστρέφει τις τιμές σαν μία λίστα και μετά χρησιμοποιώντας τον τελεστή  {\tt in}:
\index{values method}
\index{method!values}
...
\end{verbatim}
%
 Ο τελεστής  {\tt in}  χρησιμοποιεί διαφορετικούς αλγόριθμους για τις λίστες και για τα λεξικά. Για τις λίστες χρησιμοποιεί έναν αλγόριθμο αναζήτησης όπως αυτόν στην Ενότητα ~\ref{find}.  Όσο η λίστα μεγαλώνει, μεγαλώνει και 
και ο χρόνος αναζήτησης αντίστοιχα. Για τα λεξικά, η  Python  χρησιμοποιεί έναν αλγόριθμο με όνομα  
{\bf hashtable}  ο οποίος έχει μία αξιοσημείωτη ιδιότητα :  ο τελεστής  {\tt in}  χρειάζεται περίπου τον ίδιο χρόνο ανεξάρτητα από το πόσα αντικείμενα υπάρχουν μέσα σε ένα λεξικό. Δεν θα εξηγήσω πως αυτό είναι δυνατόν αλλά μπορείτε να διαβάσετε σχετικά στο σύνδεσμο  \url{http://en.wikipedia.org/wiki/Hash_table}.
\index{hashtable}
...
{\tt in}  σαν ένα γρήγορο τρόπο για να ελέγξετε εάν μία συμβολοσειρά υπάρχει μέσα στο λεξικό.

Αν κάνατε την Άσκηση ~\ref{wordlist1},  μπορείτε να συγκρίνετε την ταχύτητα αυτής της υλοποίησης με τον τελεστή  {\tt in}  όταν αυτός εφαρμόζεται σε λίστα και την δυαδική αναζήτηση.
\end{exercise}
...
\item  Μπορείτε να δημιουργήσετε 26 μεταβλητές, μία για κάθε γράμμα του αγγλικού αλφαβήτου. Έπειτα μπορείτε να διασχίσετε τη συμβολοσειρά και, για κάθε χαρακτήρα, να προσαυξάνετε τον αντίστοιχο μετρητή, χρησιμοποιώντας πιθανώς μία αλυσιδωτή συνθήκη.  

\item  Μπορείτε να δημιουργήσετε μία λίστα με 26 στοιχεία και στη συνέχεια να μετατρέψετε κάθε χαρακτήρα σε έναν αριθμό (χρησιμοποιώντας την ενσωματωμένη συνάρτηση  {\tt ord} ), να χρησιμοποιήσετε κάθε αριθμό σαν δείκτη μέσα σε μία λίστα και να προσαυξάνετε τον κατάλληλο μετρητή. 

\item  Μπορείτε να δημιουργήσετε ένα λεξικό με τους χαρακτήρες σαν κλειδιά και τις αντίστοιχες τιμές σαν μετρητές. Την πρώτη φορά που θα συναντήσετε ένα χαρακτήρα θα προσθέσετε ένα αντικείμενο στο λεξικό και έκτοτε θα αυξάνετε την τιμή ενός υπάρχοντος αντικειμένου. 
...
\end{verbatim}
%
 Το όνομα της συνάρτησης είναι  {\bf histogram},  το οποίο είναι ένας στατιστικός όρος για ένα σύνολο μετρητών (ή συχνοτήτων).
\index{histogram}
\index{frequency}
\index{traversal}

Η πρώτη γραμμή της συνάρτησης δημιουργεί ένα κενό λεξικό και ο βρόγχος  {\tt for}  διασχίζει τη συμβολοσειρά. Σε  κάθε επανάληψη, αν ο χαρακτήρας  {\tt c}  δεν είναι μέσα στο λεξικό, δημιουργούμε ένα νέο αντικείμενο με κλειδί τον  {\tt c}  και αρχική τιμή 1 (δεδομένου ότι έχουμε δει αυτό το γράμμα μια φορά). Αν ο  {\tt c}  υπάρχει ήδη μέσα στο λεξικό τότε αυξάνουμε την  {\tt d[c]}.
\index{histogram}
...
Τα λεξικά έχουν μία μέθοδο με όνομα  {\tt get}  η οποία παίρνει σαν ορίσματα ένα κλειδί και μία αρχική τιμή. 
Εάν το κλειδί εμφανίζεται μέσα στο λεξικό τότε η  {\tt get}  εμφανίζει την αντίστοιχη τιμή, αλλιώς επιστρέφει την αρχική τιμή. Για παράδειγμα :
